...
<form action="#" method="post">
	...
	<div id="container_text" style="height: auto;<? if(isset($_GET[date])) { echo 'display: none;';} ?>">
		<? if($total_events > 0 && !isset($_GET[date])) {
			...
			elseif(!isset($_GET[date])) {
				echo "You don't have any prints available yet!";
			}
			else {
				echo "&nbsp;";
			};
	...
	<div id="contained" style="height: 100%; overflow: hidden; right: 3px; border: none;">
		<?
			...
			$dir = "../clients/".$_SESSION[email]."/".$_GET[date]."/prints/thumb/";
			$originals = "../clients/".$_SESSION[email]."/".$_GET[date]."/prints/large/";
			$narray=array();
			$dh = @opendir($dir);
			$i=0;
			echo '
...
function processResults(data){
	...
	window.location.href = "?q=prints&date=" + date + "&event=" + string;
}
